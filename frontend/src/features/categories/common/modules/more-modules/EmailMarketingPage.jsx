...
export function EmailMarketingPage() {
  ...
  async function handleSendNow(c) {
    ...
    try {
      ...
      setSendResult({ id: c._id, ok: true, message: res.message || 'Sent' });
    } catch (err) {
      setSendResult({ id: c._id, ok: false, message: err.message || 'Failed to send' });
    } finally {
      setSendingId(null);
      await load();
    }
    ...
    <div className="p-4 md:p-7">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 mb-1">
        <div>
          ...
          <p className="m-0 text-[13px] text-[#536173] mt-0.5">Create and send email campaigns to a recipient list</p>
        </div>
        ...
        <form className="app-form-modal bg-white border border-[#dfe7f1] rounded-xl p-5 mt-5" onSubmit={handleSubmit}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-[#536173] font-medium">Recipients</label>
              <textarea
                className="w-full border border-[#dbe4ef] rounded-md px-3 py-2 text-[13px] outline-none focus:border-blue-500 font-[inherit] resize-y"
                placeholder="One email per line, or comma-separated"
                rows={5}
                value={form.recipientsText}
                onChange={(e) => updateForm('recipientsText', e.target.value)}
              />
            </div>
          </div>
          <div className="flex flex-col gap-1 mb-3 sm:w-64">
            <label className="text-xs text-[#536173] font-medium">Schedule send (optional)</label>
            <input
              type="datetime-local"
            ...
            />
            <p className="text-[12px] text-[#536173] m-0">Leave blank to save as a draft you send manually with "Send Now".</p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 my-5">
        {[
          ...
          { label: 'Opened',      value: stats.totalOpened.toLocaleString(),  sub: 'Tracking coming soon',                   color: '#0891b2' },
          { label: 'Clicked',     value: stats.totalClicked.toLocaleString(), sub: 'Tracking coming soon',                   color: '#7c3aed' },
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                <th className={TH}>Scheduled / Sent On</th>
            ...
            <tbody>
              ...
              {filtered.map((row) => (
                <tr key={row._id} className="hover:bg-gray-50">
                  ...
                  <td className={`${TD} text-right text-[#536173]`}>{(row.recipients ?? []).length}</td>
                  <td className={`${TD} text-[#536173]`}>{row.status === 'Scheduled' ? fmtDateTime(row.scheduledAt) : fmtDateTime(row.sentOn)}</td>
                  ...
                  <td className={TD}>
                    <div className="flex items-center gap-1">
                      <button className="w-7 h-7 flex items-center justify-center rounded hover:bg-blue-50 text-blue-500 bg-transparent border-0 cursor-pointer disabled:opacity-40" title="Send Now" type="button" disabled={sendingId === row._id} onClick={() => handleSendNow(row)}>
                        <svg fill="none" height="13" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" width="13"><line x1="22" x2="11" y1="2" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                      </button>
                    ...
                    {sendResult?.id === row._id && (
                      <p className={`text-[11px] mt-1 m-0 ${sendResult.ok ? 'text-green-600' : 'text-red-600'}`}>{sendResult.message}</p>
                    )}
                  </td>
                </tr>
              ))}

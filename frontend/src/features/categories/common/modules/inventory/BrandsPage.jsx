...
function BrandModal({ mode, initial, onSave, onClose }) {
  ...
  async function handleSubmit(e) {
    ...
    setErr('');
    try {
      const result = mode === 'add' ? await api.invCreateBrand(form) : await api.invUpdateBrand(initial._id, form);
      onSave(result);
    } catch (e) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-110 mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#edf2f7]">
          <h2 className="text-[16px] font-bold text-[#111827]">{mode === 'add' ? 'Add Brand' : 'Edit Brand'}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            <div>
              ...
              <input className={INPUT} value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="e.g. Samsung" />
            </div>
            <div>
              <label className={LABEL}>Description</label>
              <input className={INPUT} value={form.description} onChange={(e) => set('description', e.target.value)} placeholder="e.g. Electronics manufacturer" />
            </div>
            <div>
...
export function BrandsPage() {
  ...
  function loadAll() {
    ...
    Promise.all([api.invListBrands(params), api.invBrandStats()])
      .then(([brs, st]) => { setBrands(brs.data); setTotal(brs.total); setStats(st); })
  ...
  function handleDelete(id) {
    if (!window.confirm('Delete this brand? Products already tagged with it will keep the brand name as free text.')) return;
    api.invDeleteBrand(id)
      .then(() => { loadAll(); })
      .catch((e) => alert(e.message || 'Failed to delete brand'));
  }
  ...
  return (
    <div className="p-7">
      {modal && <BrandModal mode={modal.mode} initial={modal.mode === 'edit' ? modal.data : null} onSave={handleSave} onClose={() => setModal(null)} />}

      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 mb-5">
        <div>
          <h1 className="m-0 text-[22px] font-bold text-[#111827]">Brands</h1>
          <p className="m-0 text-[13px] text-[#536173] mt-0.5">Manage the brands your products belong to</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        {[
          { label: 'Total Brands',   value: stats ? stats.total.toLocaleString('en-IN') : '—', sub: 'All',            color: '#2563eb', bg: '#eff6ff', icon: <svg fill="none" height="20" stroke="#2563eb" strokeWidth="2" viewBox="0 0 24 24" width="20"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg> },
          { label: 'Active Brands',  value: stats ? stats.active.toLocaleString('en-IN') : '—', sub: 'In use',        color: '#16a34a', bg: '#f0fdf4', icon: <svg fill="none" height="20" stroke="#16a34a" strokeWidth="2" viewBox="0 0 24 24" width="20"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> },
      ...
      <div className="bg-white border border-[#dfe7f1] rounded-xl">
        <div className="flex items-center gap-3 px-5 py-3.5 border-b border-[#edf2f7]">
          <div className="relative flex-1 max-w-xs">
            ...
            <input className="border border-[#dbe4ef] rounded-md pl-8 pr-3 py-2 text-[13px] w-full outline-none focus:border-blue-500 font-[inherit]" placeholder="Search brands..." value={search} onChange={(e) => handleSearch(e.target.value)} />
          </div>
        </div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-5 py-8 text-center text-[13px] text-[#536173]">No brands found</td></tr>
              ) : brands.map((row) => (
        ...
        <div className="px-5 py-3 border-t border-[#edf2f7] flex flex-wrap justify-between gap-2 text-[13px] text-[#536173]">
          <span>Showing {brands.length === 0 ? 0 : (page - 1) * LIMIT + 1} to {(page - 1) * LIMIT + brands.length} of {total} entries</span>

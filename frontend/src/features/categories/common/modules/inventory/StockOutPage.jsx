...
function EditModal({ entry, onSave, onClose }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.to.trim()) return setErr('To (Customer / Dept.) is required');
    setSaving(true);
    setErr('');
    try {
      const payload = { ...form, itemCount: Number(form.itemCount), totalQty: Number(form.totalQty), totalValue: Number(form.totalValue) };
      const result = await api.invUpdateStockOut(entry._id, payload);
      onSave(result);
    } catch (e) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-130 mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#edf2f7]">
          <h2 className="text-[16px] font-bold text-[#111827]">Edit Stock Out</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={LABEL}>Stock Out No.</label>
              <input className={`${INPUT} bg-gray-50 text-[#536173]`} value={form.stockOutNo} readOnly />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              <label className={LABEL}>To (Customer / Dept.) *</label>
              <input className={INPUT} value={form.to} onChange={(e) => set('to', e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <SelectDropdown buttonClassName={INPUT} value={form.status} onChange={(v) => set('status', v)} options={['Pending', 'Completed']} />
            </div>
          </div>
...
export function StockOutPage() {
  ...
  function loadRecords() {
    ...
    api.invListStockOut(params)
      ...
      .catch(() => setError('Failed to load stock-out records'))
  ...
  const exportColumns = [
    { label: 'Stock Out No.', value: (row) => row.stockOutNo },
  ...
  return (
    <div className="p-7">
      ...
      <div className="mb-5">
        <h1 className="m-0 text-[22px] font-bold text-[#111827]">Stock Out</h1>
        <p className="m-0 text-[13px] text-[#536173] mt-0.5">Stock issues are recorded automatically when invoices are created in the Sales module</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        {[
          { label: 'Total Stock Out', value: stats ? stats.totalStockOut.toLocaleString('en-IN') : '—', sub: 'This Month', color: '#2563eb', bg: '#eff6ff', icon: <svg fill="none" height="20" stroke="#2563eb" strokeWidth="2" viewBox="0 0 24 24" width="20"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg> },
          ...
          { label: 'Pending',          value: stats ? stats.pending.toLocaleString('en-IN') : '—',      sub: 'To Process',  color: '#f59e0b', bg: '#fffbeb', icon: <svg fill="none" height="20" stroke="#f59e0b" strokeWidth="2" viewBox="0 0 24 24" width="20"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
        ].map((s) => (
      ...
      <div className="bg-white border border-[#dfe7f1] rounded-xl">
        <div className="flex items-center gap-3 px-5 py-3.5 border-b border-[#edf2f7] flex-wrap">
          ...
          />
          <ExportButtons title="Stock Out" filename="stock-out" rows={records} columns={exportColumns} fetchRows={fetchAllForExport} />
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className={TH}>Stock Out No.</th>
                <th className={TH}>Date</th>
                <th className={TH}>Product Name</th>
                <th className={TH}>To (Customer / Dept.)</th>
              ...
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-5 py-10 text-center text-[13px] text-[#536173]">
                  <div className="flex flex-col items-center gap-2">
                    <svg fill="none" height="32" stroke="#d1d5db" strokeWidth="1.5" viewBox="0 0 24 24" width="32"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
                    <span>No stock-out records yet. Create invoices in the Sales module to generate entries automatically.</span>
                  </div>
                </td></tr>
              ) : records.map((row) => (
                <tr key={row._id} className="hover:bg-gray-50">
                  <td className={TD}>
                    <span className="text-blue-600 font-medium">{row.stockOutNo}</span>
                    {row.sourceNo && <div className="mt-1 text-[11px] font-semibold text-blue-700 bg-blue-50 inline-flex rounded px-1.5 py-0.5">From {row.sourceNo}</div>}
                  </td>
                  ...
                  <td className={TD}>
                    <div className="flex items-center gap-1.5">
                      <button disabled={Boolean(row.sourceType && row.sourceId)} onClick={() => setEditing(row)} className="inline-flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 cursor-pointer font-[inherit] disabled:opacity-40 disabled:cursor-not-allowed"><EditIcon /> Edit</button>
                      <button disabled={Boolean(row.sourceType && row.sourceId)} onClick={() => handleDelete(row._id)} className="inline-flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-medium text-red-600 bg-red-50 border border-red-200 rounded-md hover:bg-red-100 cursor-pointer font-[inherit] disabled:opacity-40 disabled:cursor-not-allowed"><TrashIcon /> Delete</button>
                    </div>
                  </td>
                </tr>
              ))}

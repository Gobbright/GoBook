...
const UNITS = ['Strip', 'Bottle', 'Vial', 'Box', 'Piece', 'Tube', 'Sachet'];
const DEFAULT_CATEGORIES = ['Tablet', 'Syrup', 'Injection', 'Ointment', 'Drops', 'Other'];

const EMPTY_FORM = { description: '', code: '', hsn: '', category: '', brand: '', unit: 'Strip', rate: '', gstRate: 12, stock: 0, minStockLevel: 0, barcode: '', status: 'Active' };
...
function MedicineModal({ mode, initial, nextCode, initialBarcode = '', categories, onSave, onClose }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.description.trim()) return setErr('Medicine name is required');
    if (!form.rate || Number(form.rate) < 0) return setErr('Valid price is required');
    setSaving(true);
    setErr('');
    try {
      const payload = {
      ...
      setErr(e.message || 'Failed to save medicine');
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-130 mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#edf2f7]">
          <h2 className="text-[16px] font-bold text-[#111827]">{mode === 'add' ? 'Add Medicine' : 'Edit Medicine'}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="sm:col-span-2">
              ...
              <input className={INPUT} value={form.description} onChange={(e) => set('description', e.target.value)} placeholder="e.g. Paracetamol 500mg" />
            </div>
            <div>
              <label className={LABEL}>HSN / SAC</label>
              <input className={INPUT} value={form.hsn || ''} onChange={(e) => set('hsn', e.target.value)} placeholder="e.g. 3004" />
            </div>
            <div>
            ...
            <div>
              ...
              <input className={INPUT} value={form.brand || ''} onChange={(e) => set('brand', e.target.value)} placeholder="e.g. Cipla" />
            </div>
            <div>
...
export function MedicinesPage() {
  ...
  const [categories, setCategories] = useState(['All Categories', ...DEFAULT_CATEGORIES]);
  ...
  function loadCategories() {
    api.invProductCategories()
      .then((cats) => setCategories(['All Categories', ...new Set([...DEFAULT_CATEGORIES, ...cats])]))
      .catch(() => {});
  }
  ...
  async function handleScanSubmit(e) {
    ...
    try {
      ...
      if (match) {
        ...
        setCategory('All Categories');
        setPage(1);
        setScanMessage(`Found medicine: ${match.description}`);
      } else {
        setModal({ mode: 'add', nextCode: '', initialBarcode: code });
        setScanMessage('New barcode scanned. Add medicine details once to save it.');
      }
    } catch (err) {
      setScanMessage(err.message || 'Unable to scan medicine');
    } finally {
      setScanCode('');
    }
  }
  ...
  return (
    <div className="p-7">
      ...
      {pendingFile && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={(e) => e.target === e.currentTarget && setPendingFile(null)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-100 mx-4 p-6">
            <h2 className="text-[16px] font-bold text-[#111827] mb-2">Import Medicines</h2>
            <p className="text-[13px] text-[#536173] mb-5">
              Import medicines from <span className="font-medium text-[#111827]">{pendingFile.name}</span>? Existing medicines with matching codes will be updated.
            </p>
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4 mb-5">
        <div>
          <h1 className="m-0 text-[22px] font-bold text-[#111827]">Medicines</h1>
          <p className="m-0 text-[13px] text-[#536173] mt-0.5">Manage your medicine catalog and stock</p>
        </div>
      ...
      <form onSubmit={handleScanSubmit} className="mb-5 bg-white border border-[#dfe7f1] rounded-xl p-4 flex flex-col md:flex-row md:items-end gap-3">
        <div className="flex-1">
          <label className={LABEL}>Scan Medicine Barcode</label>
          <input
            className={INPUT}
            value={scanCode}
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-5 py-8 text-center text-[13px] text-[#536173]">No medicines found</td></tr>
              ) : products.map((row) => (

...
export function LoginPage() {
  ...
  async function handleGoogleOtpVerify(e) {
    e.preventDefault();
    setError('');
    setMessage('');
    setGoogleSubmitting(true);
    try {
      const user = await verifyGoogleOtpLogin({ email, otp: googleOtp });
  ...
  return (
    <LaunchExperience>
      <AuthLayout compact cardMaxWidth={400}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            <div>
              ...
              <div className="flex justify-end mt-1.5">
                <Link to="/forgot-password" className="text-[12.5px] font-semibold no-underline hover:underline" style={{ color: '#4f90ff' }}>Forgot Password?</Link>
              </div>
            </div>
        ...
        {googleOtpMode && (
          <form onSubmit={handleGoogleOtpVerify} className="flex flex-col gap-3">
            <button type="button" onClick={resetGoogleOtpMode} className="inline-flex items-center gap-1.5 self-start text-[12.5px] font-semibold text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 bg-transparent border-0 cursor-pointer p-0 mb-1">
            ...
            <div className="text-center mb-1">
              ...
              <p className={`text-[12px] m-0 break-all ${MUTED}`}>{email}</p>
            </div>
            <input value={googleOtp} onChange={(event) => setGoogleOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} className={`${INPUT} pl-3 text-center tracking-[8px] text-[20px] font-extrabold`} placeholder="000000" inputMode="numeric" autoComplete="one-time-code" maxLength={6} required />
            {error && <div className={`rounded-xl px-4 py-2.5 ${ERROR_BOX}`}><span className={`text-[12.5px] ${ERROR_TEXT}`}>{error}</span></div>}
            {message && <div className="rounded-xl px-4 py-2.5 bg-emerald-50 text-emerald-700 text-[12.5px] border border-emerald-100">{message}</div>}
            <button type="submit" disabled={googleSubmitting || googleOtp.length !== 6} className="w-full flex items-center justify-center gap-2.5 rounded-xl px-4 py-3 text-white font-bold text-[14px] border-0 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" style={{ background: 'linear-gradient(135deg, #4f90ff 0%, #6366f1 100%)' }}>
              <CheckCircle2 size={17} />{googleSubmitting ? 'Verifying...' : 'Verify and Continue'}
            </button>
            <button type="button" onClick={handleGoogleOtpStart} disabled={googleSubmitting} className="w-full flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold text-[12px] cursor-pointer disabled:opacity-60">
              <RefreshCw size={14} />{googleSubmitting ? 'Sending...' : 'Resend OTP'}
            </button>
          </form>
        )}

      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 text-slate-900 dark:text-slate-100 dark:text-slate-100">
        <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 py-5">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="m-0 text-2xl font-extrabold text-slate-900 dark:text-slate-100">{config.title}</h1>
              <p className="m-0 text-sm text-slate-500 dark:text-slate-400 mt-1">{config.subtitle}</p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button className="px-4 py-2 bg-slate-900 text-white rounded-lg flex items-center gap-2 hover:bg-slate-800 transition border-0 cursor-pointer">
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
              ...
              <p className="m-0 text-2xl font-extrabold text-slate-900 dark:text-slate-100 mt-1">{totalRecords}</p>
            </div>
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
              <p className="m-0 text-sm text-slate-500 dark:text-slate-400 font-medium">DB Modules</p>
              <p className="m-0 text-2xl font-extrabold text-blue-600 dark:text-blue-400 mt-1">{sections.length}</p>
            </div>
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4">
              <p className="m-0 text-sm text-slate-500 dark:text-slate-400 font-medium">Visible Tables</p>
              <p className="m-0 text-2xl font-extrabold text-emerald-600 dark:text-emerald-400 mt-1">{filteredSections.length}</p>
            </div>
          </div>
